...
public class CustomCSSSelector {
	...
	//css selector -- locator approach
			...
			/*
			 Task:
				a.	Navigate to https://app.hubspot.com/login
				b.	Pretend that "Email address", "Password", "Log in", and "Remember me" are important 
					elements and store them in one selector (rather than creating four By locators for it).
				c.	Verify if these elements are present in the page or not.

			 */
			
			int loginEles = driver.findElements(By.cssSelector("#username,#password,#loginBtn,#checkbox-content-4")).size();
			System.out.println("Number of important elements in Login page: " + loginEles);
			if(loginEles == 4) {
				System.out.println("All important elements are present in login page --- PASS");
			}else {
